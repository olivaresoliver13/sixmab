...
@section('titulo')
    ...
    <div class="ico-sixmab1">
        ...
        <span class="ml-2">Regeneración</span>
    </div>
    <div class="w-100 alto1 alto2"> 
        @can('regeneracion.create')
            <div class="box-header with-border">            
                <a href="{{route('baterias.regeneracion.create', ['bateria' => $bateria->id])}}" class="btn btn-success btn-sm pull-right rounded-pill text-white class-volver" style="width: 130px;"><i class="fa fa-fw fa-plus-circle mr-1 tam-ico"></i><span id="bottom-ico">{{ __('adminlte::adminlte.create-record') }}</span></a>
            </div>
        @endcan   
                    ...
                    <div class="col-12">
                        ...
                        <div class="card-body">
                            <div class="container" style="max-width: 90%;">
                                <div class="main">
                                    @foreach ($regeneracion as $item)
                                        <ul class="cbp_tmtimeline">
                                            <li>                                            
                                                <time class="cbp_tmtime" datetime="2013-04-10 18:30"><span>{{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y')}}</span> <span>{{ \Carbon\Carbon::parse($item->created_at)->format('H:i:s')}}</span></time>
                                                <div class="cbp_tmicon"></div>
                                                <div class="cbp_tmlabel">
                                                    <table>
                                                        <tbody>
                                                            <tr>
                                                                @can('regeneracion.destroy')
                                                                    <div class="box-header">            
                                                                        <form action="{{route('regeneracion.destroy', ['regeneracion' => $item->id])}}" class="d-inline form-eliminar" method="POST">
                                                                            @csrf @method("delete")
                                                            ...
                                                            <tr>
                                                                @can('regeneracion.edit')
                                                                    <div class="box-header">            
                                                                        <a href="{{route('regeneracion.edit', ['regeneracion' => $item->id])}}" class="pull-right tooltipsC" title="Editar Registro" style="border: 0; background: transparent; margin-right: 12px; color: #ffc107;">
    ...
    @else
        ...
        <div class="alert alert-info alert-dismissible" style="width:100%">
            ...
            <font style="vertical-align: inherit;">
                ...
                <font style="vertical-align: inherit;"><br>Por favor ingrese una Regeneración.</font>
            </font>
        </div>
    @endif
@stop