...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <input type="text" data-toggle="tooltip" title="Ingrese el Nombre de la Empresa" class="form-control" name="nombre" id="nombre" placeholder="Nombre" value="{{old('nombre', $data->nombre ?? '')}}" minlength="1" maxlength="100" required/>
  </div>
</div>
<div class="form-group row">
...
<div class="form-group row">
  ...
  <div class="col-sm-10">
    <textarea data-toggle="tooltip" name="direccion" id="direccion" class="form-control" rows="1" id="comment" title="Ingrese la Dirección de la Empresa" placeholder="Dirección" minlength="1" maxlength="500" required>{{old('direccion', $data->direccion ?? '')}}</textarea>
  </div>
</div>
<div class="form-group row">
  <label for="tipo_paso_sixmab" class="col-sm-2 col-form-label requerido text-left">Paso SIXMAB</label>
  <div class="col-sm-10">
    <select class="selectpicker form-control" multiple data-actions-box="true" data-selected-text-format="count > 4" data-style="bg-white" data-live-search="true" title="Seleccione Pasos SIXMAB" name="tipos_paso_sixmab[]" id="tipo_paso_sixmab">
      @foreach ($tipos_paso_sixmab as $paso_sixmab)
        @isset($data)
          <option value="{{$paso_sixmab->id}}" {{$pasos_empresa->contains('tipo_paso_sixmab_id', $paso_sixmab->id) ? 'selected' : ''}}>{{$paso_sixmab->nombre}}</option> 
        @else
          <option value="{{$paso_sixmab->id}}">{{$paso_sixmab->nombre}}</option>
        @endisset
      @endforeach
    </select>
  </div>
